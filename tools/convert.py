...
from tools.layout import Box, collect_svg_boxes
...
class ConvertContext:
    ...
    def begin_element(
        ...
        if tag == "path" and element.get("data-primitive-kind") == "brace":
            from tools.primitives import materialize_brace, transform_brace_spec
    ...
    def attach_asset_contracts(
        ...
        from tools.asset_spec import AssetSpecError, asset_spec_sha256, attach_asset_specs
...
def _convert_in_place(run: common.Run) -> dict:
    from tools.contracts import initialize_contracts, read_json, transition
    from tools.revisions import bind_canonical_svg, materialize_svg, read_svg_text_exact
...
def _validate_shadow_conversion(run: common.Run, summary: dict) -> None:
    ...
    from tools.asset_spec import asset_contract_sha256, canonical_asset_contract_payload
...
def convert(run: common.Run) -> dict:
    ...
    from tools.transactions import publish_staged_files, staged_case_copy
