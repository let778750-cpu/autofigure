...
def create_run(
    ...
    *,
    ...
    from tools.core.contracts import INPUT_ROUTES, PROCESSING_MODES
...
def open_run(run_dir: Path) -> Run:
    ...
    run = Run(resolved)
    run.load_meta()
    from tools.core.contracts import ContractError, initialize_contracts, validate_reference
...
def main() -> int:
    sys.stdout.write("tools.core.common 是库模块，请使用 autofigure <prepare|convert|check|math>。\n")
    return 0
