...
| `manual_asset_slot` | 照片级/复杂不可原生 | `creative_raster` → 占位槽交用户（见 06） |
...
| `creative_raster` | 照片级/复杂不可原生（显微/3D/照片/复杂图标） | **占位 slot 交用户填充**（见 06） |
| `forbidden` | 含正式文字/公式/箭头/拓扑/坐标轴/定量证据/整图背景的位图 | 禁止，拆成可编辑对象 |

## 2. Atomic Raster（原子位图）硬约束

- 每张保留位图 = **一个语义单一的不可再分视觉场**（`atomic_raster_unit=true`）。
...
- `creative_raster` → 走 06 的占位 slot（交用户）；`forbidden` → 拆可编辑；`exact_vector`/`auditable_concept` → codex 原生画。
