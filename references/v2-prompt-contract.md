...
- 箭头 marker 的 refX 未对齐三角尖端、头长与线宽比例失调、端点悬空不落形状边缘 → `autofigure arrows` 会逐条审计；几何偏差可用 `arrows --fix` 确定性修复后重跑 convert。
