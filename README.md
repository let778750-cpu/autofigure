...
GPT 网页端直出 + 第二圈自批评修订（箭头实心化贴原图、照片原子裁剪）+ 箭头结构确定性修复（42 处锚点归零、曲线箭头按末端切线对齐）。243 个原生对象全部可编辑，22 个公式（𝔼、∇、上下标等）升级为原生 Office Math。
...
```
→ 审计箭头锚点 / 头线比例 / 端点悬空并汇入 check 报告；`--fix` 只动几何不动样式，`--clamp-ratio` 头长限幅。
