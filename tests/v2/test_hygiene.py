"""tools.v2.hygiene 的规则、豁免与 CLI 测试。"""
...
def test_fix_before_after_narrative_flagged(tmp_path):
    _write(tmp_path, "QA_STATUS_V3.md", "- 修复前:越界 `27.96 px`;修复后:越界 `0 px`。\n")
...
def test_round_fix_restricted_to_status_reports(tmp_path):
    _write(tmp_path, "QA_STATUS_V3.md", "本轮布局修复全部归零。\n")
